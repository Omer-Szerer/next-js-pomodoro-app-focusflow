...
// Utility function for authentication checks
...
export async function PATCH(request: Request): Promise<NextResponse> {
  try {
    ...
    // Now we can safely call the updateSubtaskCheckedStatus function
    const updatedSubtask = await updateSubtaskCheckedStatus(
      sessionTokenCookie, // sessionTokenCookie is now guaranteed to be a string
      subtaskId,
      checked,
    );
